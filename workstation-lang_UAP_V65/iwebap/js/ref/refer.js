...
//                }
    ...
    function findPos(obj, refObj) {
        ...
        var billItemWidth = refObj.billItemWidth || 175;
    ...
    var viewHelper = {
		...
		//树的样式
        navName: function (isLast) {
            //return isLast ? '' : ' >';
            return isLast ? '' : '<img src="/iwebap/images/canzhao_1.png" class="folder" style="width: 7px;height: 10px;vertical-align: baseline;">';
        },
		//非叶节点(父节点)是否选中
...
//            }
    ...
    Refer.prototype.refreshHotData = function (isClick) {
		...
		if (that.isQuerytpl()) {
			...
			var hotTpl='<li data-initial="hotData" class="curr">Favourite</li>';
    ...
    Refer.prototype.place = function () {
        ...
        var pos = findPos($input[0], that);
        that.$el.find('.outerContainer').css({
            top: (pos.y1 + $input[0].offsetHeight) + "px",
            left: pos.x1 + "px"
        });
			...
			if(that.isGridTree()){
				var tplItem = '\
						...
						var tplItem = '\
		                        <% _.each(refTreeNodeList,function(ref,index){%>\
		                              <li class="openLi" title="<%-getCustomizeTips(ref)%>" id="<%=ref.refpk%>" data-value="<%=ref.refpk%>" pk="<%=ref.refpk%>"  data-name="<%=ref.refname%>" data-code="<%=ref.refcode%>" isLeaf="<%=ref.isLeaf%>" >\
		                                 <%=notLeafSelectedclass()?\'<img src="/iwebap/images/danxuan_1.png" class="selected" style="float:left;margin-top:4px;width: 16px;height: 16px;">\':""%>\
										 ...
										 <%=ref.isLeaf?"":\'<img src="/iwebap/images/canzhao_1.png" class="folder" style="float:right;margin-top:7px;width: 7px;height: 12px">\' %>\
		                              </li>\
		                        <% }) %>\
		                     ';
					
						var hasHotData=that.$el.find('ul.valueList').hasClass('hotData');
						var hotTpl=hasHotData?'<li data-initial="hotData">Favourite</li>':'';
						var navItemTpl = hotTpl+'\
									<% _.each(refTreeNavList,function(ref,index){%>\
						                <li class="<%=(_.size(refTreeNavList)===(index+1))?\'curr\':\'\'%>" pk="<%=ref.refpk%>" data-initial="<%=ref.id%>"><%-ref.refname%><%=navName(_.size(refTreeNavList)===(index+1)) %></li>\
									<% }) %>\
									';
					...
					that.$el.find('ul.valueList').on('click', 'li.openLi', function(e){
						...
						var $that = $(this);
						$that.find('img.folder').attr("src", "/iwebap/images/canzhao_2.png");
						$that.find('img.selected').attr("src", "/iwebap/images/danxuan_2.png");
					}).on('mouseleave', 'li.openLi', function(){
						var $that = $(this);
						$that.find('img.folder').attr("src", "/iwebap/images/canzhao_1.png");
						$that.find('img.selected').attr("src", "/iwebap/images/danxuan_1.png");
					});
				...
				var classTplItem = '\
					<li data-initial="0" class="curr">All</li>\
...
//            }
        ...
        that.$el.find('.refer_results').on('click', '.refer_list li',function (e) {
			...
			TimeFn = setTimeout(function() {
                ...
                $that.find('img.folder').attr("src", ""+ctx+"/images/canzhao_2.png");
                $that.find('img.selected').attr("src", ""+ctx+"/images/danxuan_2.png");
            }).on('mouseleave', '.refer_list li', function () {
                var $that = $(this);
				var ctx=that.options.ctx;
                $that.find('img.folder').attr("src", ""+ctx+"/images/canzhao_1.png");
                $that.find('img.selected').attr("src", ""+ctx+"/images/danxuan_1.png");
                    ...
                    <div class="innerContainer" style="border:1px solid #c8cac9">\
                            <div class="refer_results" style="height:305px;">\
                                    <ul class="refer_nav clx tab" >\
                                        ...
                                        <li index="2" condition="" class="nav_list2 allData" ><span>All</span></li>\
                    ...
                    <ul class="refer_nav clx navtree">\
                       <% _.each(refTreeNavList,function(ref,index){%>\
                         <li index="<%=(index+1)%>"   condition="<%=ref.refpk%>"  class="nav_list<%=(index+1)%> <%=(_.size(refTreeNavList)!==(index+1))?\' action\':\'\'%>" >\
                              <span><%-ref.refname%></span><%=navName(_.size(refTreeNavList)===(index+1)) %>\
                         </li>\
                        <% }) %>\
                      </ul>\
                        ';
                       ...
                       <ul class="refer_list ul_list2 treeNodeStyle" style="overflow: auto; max-height: 271px; height: 271px; display: none;padding-top: 10px;">\
                        <% _.each(refTreeNodeList,function(ref,index){%>\
                              <li class="openLi" title="<%-getCustomizeTips(ref)%>" name="<%=ref.refname%>" id="<%-ref.id%>" pid="<%-ref.pid%>" pk="<%-ref.refpk%>" code="<%-ref.refcode%>" isLeaf="<%=ref.isLeaf%>" >\
                                 <%=notLeafSelectedclass()?\'<img src="/iwebap/images/danxuan_1.png" class="selected" style="float:left;margin-top:3px;width: 16px;height: 16px;background-color: #ffffff;">\':""%>\
                                 ...
                                 <%=ref.isLeaf?"":\'<img src="/iwebap/images/canzhao_1.png" class="folder" style="width: 7px;height: 12px">\' %>\
                              </li>\
                        <% }) %>\
                    </ul>\
                    ';
