	...
	$("#"+name).each(
			function() {
				...
				var options = {
					  ...
					  dom: $that,
					  pageUrl: '/iwebap/js/ref/refDList.js',
					...
					function() {
						...
						if (param_note != null && param_note.length > 50) {
							$.showMessageDialog({
								type : "info",
								title : "提示",
								msg : "Approval can only enter 50 words or less",
								backdrop : true
							});
							return;
						}
						...
						if (actionCode === 'reject') {
							if (param_note == null
								...
								$.showMessageDialog({
									type : "info",
									title : "提示",
									msg : "Reject the process must enter comments",
									backdrop : true
								});
								return;
							}
...
dialogActionReassign = function (action) {
		...
		if (user === undefined ) {
			$.showMessageDialog({
				type : "info",
				title : "提示",
				msg : "Please select the specific assignee",
				backdrop : true
			});
			return;
		}
...
dialogActionAddapprove = function (action) {
	...
	if (user === undefined ) {
		$.showMessageDialog({
			type : "info",
			title : "提示",
			msg : "Please select a specific signer",
			backdrop : true
		});
		return;
	}
...
dialogActionReject = function (action) {
	...
	if ($("#datatable").find("tr").hasClass("datarow-choosen")) {
		...
		$.showMessageDialog({
			type : "info",
			title : "提示",
			msg : "Please select a specific link",
			backdrop : true
		});
		return;
	}
...
function historyTaskShowMoreNew(note, org) {
	...
	approveInfo += '        </div>';
	approveInfo += '    </div>';
	approveInfo += historyTaskViewRight(note, null, org);
	approveInfo += '</div>';
	return approveInfo;
 ...
 */
function historyTaskWithIconNew(note, todo_actions, pk_workflownote, org) {
	var approveInfo = '';
	if (note.actiontype == 'agree') {
		...
		// 审批不通过
		approveInfo += ' <div class="approve-unpassed" pk_workflownote="'
				+ pk_workflownote
				+ '" ><div class="approve-left"> <div class="approve-bage"><span class="fa fa-close"></span></div></div>';
	...
	approveInfo += historyTaskViewRight(note, todo_actions, org, true);
	approveInfo += '</div>';
	return approveInfo;
}
 ...
 * @returns {string}
 */
function historyTaskViewRight(note, todo_actions, org) {
	return historyTaskViewRight(note, todo_actions,org, false);
}

function historyTaskViewRight(note, todo_actions, org, isLast) {
	var approveInfo = '<div class="approve-right">';
	approveInfo += '    <p><div class="approve-person" title="'
			+ note.checkman + '">' + note.checkman
			+ '</div><div class="approve-timespan">'
			+ getFormatDateByLong(note.dealDate) + '</div></p>';
	if (!isLast) {
		approveInfo += '     <div class="approve-horizon-line"></div>';
	}
	var checknotetext = typeof (note.checknote) != "undefined" ? note.checknote
			: "";
	...
	approveInfo += '     <p><em class="approve-action">' + note.actionname
 ...
 * @param dom 前台元素
 */
function getApproveInfosNew(openbillid, billtype, pagedom, org, callback) {
			
			var $approvaouter = $(pagedom);
			$approvaouter.html('<br><div class="grid simple jbxx"><div class="grid-title"><h4 class="label-left-em" id="approvalSituation">审批情况</h4> </div> <div class="grid-body"><div id="apprvoeinner" class="form-wizard-steps approve approve-ver " style="min-height: 90px;"></div></div></div>');
			
			var dom = $('#apprvoeinner');

			$.ajax({
				url : '/iwebap/wf_ctr/workflownoteQry?billID=' + openbillid
				...
				success : function(json) {
					...
					// 流程是否已经结束
					var hasTodo = todo_action_info.actions != null && todo_action_info.actions.length > 0;
					...
					if (hasTodo) {
						if (noteLength > 2) { // 如果已处理的任务数>2的话,那么需要输出浮出层样式
							...
							if (todo_users != null && todo_users.length != 0) {
								// 最后一个已处理任务中的流程
								approveInfo += historyTaskWithNumNew(lastOneTask, noteLength, org);
								// if()
								// 如果有待处理人,则显示待处理人的样式
								...
								approveInfo += historyTaskWithIconNew(lastOneTask, todo_action_info.actions,todo_action_info.pk_workflownote);
							}
							// 处理待处理的流程
						} else { // 如果已处理的任务数<=2
							// 第一个
							...
							if (todo_users != null && todo_users.length != 0) {
								...
								approveInfo += historyTaskWithIconNew(notes[noteLength-1],todo_action_info.actions,todo_action_info.pk_workflownote);
							}
						}
					} else {
						// 2. //如果没有待处理的任务
						if (noteLength > 2) {
							...
							approveInfo += historyTaskWithNumNew(notes[0], 1, org);
							if (noteLength == 2) {
								approveInfo += historyTaskWithIconNew(notes[1]);
							}
						}
					}
					approveInfo += '        </div>';
...
function interpreTodoBtnsNew(action, org) {
	...
	if (action.action_code == 'agree') {
		...
		approveInfo += '<button id="turndown" class="approve-btn" type="button"  action-code="'
		...
		approveInfo += '<button id="cancelapproval" class="approve-btn approve-btn-first "  type="button" action-code="'
	...
	if (todo_actions.length > 2) {
		...
		for ( var beginIndex = 2; beginIndex < todo_actions.length; beginIndex++) {
			...
			if (todo.action_code == 'agree') {
				...
				approveInfo += '<li><a id="notapproval" href="javascript:void(0);" assign-flag="' + todo.assign_flag + '" action-code="' +
...
function todoListsViewCtrlNew(todo_action_info, todos, waittime, wait_commit, org) {
	...
	if (todo_action_info.showCheckNote) {
		approveInfo += '    <textarea id="inputinfomsg" class="form-control approve-textarea-scale" rows="2" style="height: 48px;" placeholder="请输入意见"></textarea>';
	} else if (todos != null && todos.length != 0) {
		approveInfo += '      <div class="approve-person-pending"> <p>Wait';
		var title = 'Wait: ';
		...
		approveInfo += (wait_commit ? 'Submit' : 'Approve') + '</p><p>In the hands of time' + waittime
				+ '</p> </div>';
	}
	var todo_actions = todo_action_info.actions;
	approveInfo += '</div>';
	// 按钮的生成
	approveInfo += generateTodoButtonNew(todo_actions,org);
	return approveInfo;
...
function generateReassignView(pk_workflownote, actionCode, param, org) {
		...
		view += '                  <div class="modal-header reject-header"><h4>改派</h4><a class="reject-head-close" title="点击关闭" href="javascript:void(0)" data-dismiss="modal"><img src="/iwebap/images/workflow/close.png" class="dialog-close"/></a></div>';
...
function generateAddapproveView(pk_workflownote, actionCode, param, org) {
		var view = '<div class="modal-dialog" workflow_note="' + pk_workflownote + '" actionCode=' + actionCode + ' param="' + param + '" >';
		view += '           <div class="modal-content reject-content">';
		view += '                  <div class="modal-header reject-header"><h4>加签</h4><a class="reject-head-close" title="点击关闭" href="javascript:void(0)" data-dismiss="modal"><img src="/iwebap/images/workflow/close.png" class="dialog-close"/></a></div>';
		view += '                  <div class="modal-body"  style="background-color: white;padding-left:0; padding-right:0; padding:0px">';
		view += '                  <fieldset class="form-group form-group-sm billitem">	';										
		view += '                  		<label class="billitem-label" style="color:#4C4C4C">';
		view += '                  			<div class="billitem-label-content">业务员</div>';
...
function generateRejectView(pk_workflownote, actionCode, param) {
	$
			.ajax({
				url : '/iwebap/wf_ctr/getRejectActivity?pk_workflownote='
				...
				success : function(rejectInfos) {
					...
					if (rejectInfos.message) {
						$.showMessageDialog({
							type : "info",
							title : "提示",
							msg : "No information is found, The data may have been updated. Please refresh the document",
							backdrop : true
						});
						return;
					}
					var view = '     <div class="modal-dialog" workflow_note="'
							+ pk_workflownote + '" actionCode=' + actionCode
							+ ' param="' + param + '" >';
					view += '           <div class="modal-content reject-content">';
					view += '                  <div class="modal-header reject-header"><h4>驳回至</h4><a class="reject-head-close" title="点击关闭" href="javascript:void(0)" data-dismiss="modal"><img src="/iwebap/images/workflow/close.png" class="dialog-close"/></a></div>';
					view += '                  <div class="modal-body"  style="background-color: white;padding-left:0; padding-right:0; padding:0px">';
					view += '                      <table class="table reject-table" id="datatable"><tbody>';
					for ( var i = 0; i < rejectInfos.length; i++) {
						var rejectInfo = rejectInfos[i];
						var trLine = '<tr  class="reject-tr" activity-def-id="'
								+ rejectInfo.activityDefId + '">';
						trLine += '<td class="reject-person-td">' + rejectInfo.activityDesc + '</td>';
						var notes = rejectInfo.workflownotes;
						if (notes.length > 1) {
							var subtable = '<td class="person" colspan="2">';
							var names = '';
							var sep = "";
							for ( var k = 0; k < notes.length; k++) {
								names += sep;
								names += notes[k].checkman;
								sep = "、";
							}
							subtable += ' <span class="names"  title="' + names
									+ '">' + names + '</span>';
							subtable += ' <div class="subtable" style="display:none">';
							subtable += '   <table class="table  reject-sub-table">';
							subtable += '                 <tbody>';
							for ( var j = 0; j < notes.length; j++) {
								var note = notes[j];
								subtable += '<tr>';
								subtable += '   <td>' + note.checkman + '</td>';
								subtable += '   <td>' + note.actionname
										+ '</td>';
								subtable += '   <td>'
										+ (typeof (note.checknote) != "undefined" ? note.checknote
												: '') + '</td>';
								subtable += '</tr>';
							}
							subtable += '           </tbody>';
							...
							subtable += '</div>';
							subtable += '</td>';
							subtable += ' <td><span class="reject-arrow-down fa fa-caret-square-o-down"></span></td>';
							trLine += subtable;
						} else {
							trLine += '<td class="reject-table td">' + notes[0].checkman + '</td>';
							trLine += '<td class="reject-table td">' + notes[0].actionname + '</td>';
							trLine += '<td class="reject-table td">'
									+ (typeof (notes[0].checknote) == 'undefined' ? ''
											: notes[0].checknote) + '</td>';
						}
						trLine += '</tr>';
						view += trLine;
					}
...
function generateAssignView(pk_workflownote, actionCode, param_note) {
	$
			.ajax({
				url : '/iwebap/wf_ctr/getAssignInfo?pk_workflownote='
						+ pk_workflownote + "&ramdom=" + new Date().getTime(),
				"type" : "GET",
				dataType : 'json',
				beforeSend : $.showWaiting(),
				success : function(assignInfos) {
					...
					view += '               <div class="modal-header assign-header"><h4 class="modal-title assign-title">指派<a href="javascript:void(0)" data-dismiss="modal"><img src="/iwebap/images/workflow/close.png" class="dialog-close"/></a></h4></div>';
...
/**
 * 生成审批信息的右面的部分,因为他们都是统一的样式,所以使用一个统一的函数输出即可,增加代码的复用性
 */
function generateApproveRightInfo(note) {
	var displayName = note.checkman;
	if (note.checkman !== undefined && note.checkman.length>15 ) {
		displayName = note.checkman.substr(0,10) + "...";
	}

	var approveInfo = '      <div class="approve-right">';
	approveInfo += '            <p style="margin-bottom:5px;height:15px;margin-top:5px"><span class="approve-person" title="'
			+ note.checkman + '">' + displayName
			+ '</span> <span class="approve-timespan">'
			+ getFormatDateByLong(note.dealDate) + '</span> </p>';
	var checknote = typeof (note.checknote) == 'undefined' ? ''
			: note.checknote;
	...
	approveInfo += '            <p style="margin-bottom:15px;height:15px"><em class="approve-action">'
			+ note.actionname + '</em><span class="approve-note" title="'
...
function doAction(pk_workflownote, actionCode, param) {
	...
	$.ajax({
		...
		url : '/iwebap/wf_ctr/doAction?pk_workflownote=' + pk_workflownote
		...
		success : function(data) {
			...
			if (typeof (data.message) == "undefined") {
				...
				$.showMessageDialog({
							type : "info",
							title : "提示",
							msg : data.message != null ? data.message : "An unknown reason has occurred and the approval failed",
							backdrop : true
						});
			}
		}
...
function getBillFlowInfos(billid, billtype,billNo, dom) {
	...
	var billUrl="/iwebap/flowinfoctrl/queryflow?openbillid=" + billid
	+ "&billtype=" + billtype + "&time="
	+ new Date().getTime();
							...
							for ( var i = 0; i < datas.length; i++) {
								...
								tds += '<tr><td class="text-center" style="text-align:left">';
									...
									if(typeof billNo === 'undefined' || billNo == null){
										...
										+ data.tradetypename
										+ '</td><td class="text-center" style="text-align:left"><a href="/iwebap/flowinfoctrl/redirect?openbillid='
...
define("workflow", ["dateutil"], function(){});


define("wfcomp", function(){});
